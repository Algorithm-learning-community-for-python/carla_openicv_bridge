...
"""
base class for spawning a Ego Vehicle in 

Two modes are available:
- spawn at random Carla Spawnpoint
- spawn at the pose read from  topic /initialpose
    ...
    def __init__(self):
        ...
        if self.spawn_point_param and self.spawn_ego_vehicle:
            ...
            if len(spawn_point) != 6:
                raise ValueError("Invalid spawnpoint '{}'".format(self.spawn_point_param))
            pose = Pose()
        ...
        self.initialpose_subscriber = Subscriber(
            "/carla/{}/initialpose".format(self.role_name))#            PoseWithCovarianceStamped,            self.on_initialpose
